...
9. [문제 해결](#9-문제-해결)
10. [LLM 연동 — L4 문장 조화 분석 (선택 사항)](#10-llm-연동--l4-문장-조화-분석-선택-사항)
...
## 10. LLM 연동 — L4 문장 조화 분석 (선택 사항)

규칙으로는 잡기 어려운 중복 표현·복잡한 문장·피동 남용을 AI가 추가로 분석합니다.
LLM을 설정하지 않아도 공문닥터는 정상 동작합니다.

### 1단계: llama-cpp-python 설치
...
> **설치 실패 시:** [llama-cpp-python 릴리스 페이지](https://github.com/abetlen/llama-cpp-python/releases)에서
> 본인 Python 버전에 맞는 `.whl` 파일을 내려받아 직접 설치하세요.
> ```
...
아래 두 모델 중 하나를 선택합니다. RAM 4GB 이하 PC는 EXAONE, 여유 있으면 Qwen을 권장합니다.
...
HuggingFace 검색창에 모델명을 입력하고 `Q4_K_M.gguf` 파일을 찾아 내려받습니다.

예시 저장 위치: `C:\models\EXAONE-3.5-2.4B-Instruct-Q4_K_M.gguf`

> `Q4_K_M`은 4-bit 양자화 버전으로, 품질과 속도의 균형이 좋습니다.
...
```
[LLM] 모델 로드 중: C:\models\EXAONE-3.5-2.4B-Instruct-Q4_K_M.gguf
[LLM] 모델 로드 완료 (L4 문장 조화 분석 활성화)
[1/4] 규칙 로드 완료: 69개
[2/4] 원본 백업: 문서.20260322_143015.bak.hwpx
[3/4] 문서 열기 완료: 문서.hwpx (21개 문단)
[4/4] 교정 파일 저장: 문서_corrected.hwpx
...
> L4 제안은 문서를 직접 수정하지 않습니다. 사람이 검토 후 적용 여부를 결정하세요.

### 자주 묻는 질문 (LLM)

**Q. 모델 로드에 시간이 오래 걸려요.**

A. 처음 로드 시 20~60초가 걸릴 수 있습니다. 이후 교정 처리는 문단당 2~5초입니다.

**Q. "llama-cpp-python이 설치되지 않았습니다" 오류가 나요.**

A. `pip install llama-cpp-python --prefer-binary`를 다시 실행하세요.
안 되면 `--llm-model` 옵션 없이 사용하면 됩니다.

**Q. LLM 없이도 쓸 수 있나요?**

A. 네. `--llm-model` 옵션을 빼면 기존 L1-L3 규칙 교정만 적용됩니다.
