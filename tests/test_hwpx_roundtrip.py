...
class TestRoundTrip:
    def _roundtrip(self, hwpx_path: Path, workspace_tmpdir: Path) -> tuple[str, str, int]:
        """Open → correct → save → re-open. Returns (original_text, new_text, count)."""
        tmp_file = workspace_tmpdir / hwpx_path.name
    ...
    def test_road_repair_roundtrip(self, road_repair_hwpx: Path, workspace_tmpdir: Path):
        original, corrected, count = self._roundtrip(road_repair_hwpx, workspace_tmpdir)
        # PoC verified 12 corrections on this document
        assert count > 0, "Expected corrections on road repair document"

    def test_completion_inspect_roundtrip(self, completion_inspect_hwpx: Path, workspace_tmpdir: Path):
        original, corrected, count = self._roundtrip(completion_inspect_hwpx, workspace_tmpdir)
        assert count >= 0  # may be 0 after prior runs

    def test_cooperation_request_roundtrip(self, cooperation_request_hwpx: Path, workspace_tmpdir: Path):
        original, corrected, count = self._roundtrip(cooperation_request_hwpx, workspace_tmpdir)
        assert count >= 0

    def test_dry_run_does_not_modify_file(self, road_repair_hwpx: Path, workspace_tmpdir: Path):
        tmp_file = workspace_tmpdir / road_repair_hwpx.name
